...
Fixed table names to match actual Supabase schema (all lowercase).
"""
import hashlib
...
def get_driver_by_id(driver_id: str):
    try:
        ...
        return (
            ...
            r.get("driver_id"), None, None, None, 0, None, None,
...
def get_last_fuel(truck_id):
    try:
        ...
        if not client:
            return None
        row = client.table("fuel_consumption").select(
...
def get_driver_docs(truck_id, driver_id, limit=30):
    try:
        ...
        if not client:
            return []
        rows = client.table("driver_documents").select(
...
def save_fuel(d: dict) -> bool:
    try:
        ...
        if not client:
            return False
        client.table("fuel_consumption").insert({
            "truck_id": d["truck_id"],
            "date": d["date"],
...
def save_event(d: dict) -> bool:
    try:
        ...
        if not client:
            return False
        client.table("maintenance_log").insert({
            "truck_id": d["truck_id"],
            "date": d["date"],

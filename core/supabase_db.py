...
def save_trip(d):
    try:
        ...
        svc = sb.table("truck").select(
            "last_service_km,service_interval,service_warning_active"
        ).eq("truck_id", d["truck_id"]).limit(1).execute()
        if svc.data and odo > 0:
            row = svc.data[0]
            ls = row.get("last_service_km") or 0
            iv = row.get("service_interval") or 15000
            ac = row.get("service_warning_active", False)
            if (odo - ls) >= iv * 0.90 and not ac:
                sb.table("truck").update({
                    "service_warning_active": True,
                    "service_warning_date": d["date"],
                }).eq("truck_id", d["truck_id"]).execute()
                try:
                    sb.table("service_warning").insert({
                        "truck_id": d["truck_id"],
                        "warning_type": "Service Due",
                        "triggered_date": d["date"],
                        "triggered_km": odo,
                    }).execute()
                except Exception:
                    pass
                st.warning("⚠️ Service warning triggered.")
        return True
    except Exception as e:

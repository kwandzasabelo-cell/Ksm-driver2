...
def get_driver_by_id(did):
    ...
    try:
        ...
        row = conn.execute("""SELECT truck_id,registration,driver,mileage,fuel_tank_capacity,
            ...
            driver_routes,driver_certifications,truck_status,model FROM Truck WHERE driver_id=?""",(did,)).fetchone()
    ...
    try:
        ...
        cur.execute("""INSERT INTO Trip
            ...
            (d["truck_id"],d["origin"],d["destination"],dist,d.get("load_kg",0),d["date"],
             fuel,eff,d.get("duration_h",0),d.get("border_crossings",0),
...
with tab_prof:
    ...
    if not tid or not sel_row: st.info("Select a truck above.")
        ...
        if sp and connected:
            try:
                conn=get_conn()
                conn.execute("UPDATE Truck SET driver_phone=?,driver_license=?,driver_id_number=?,driver_certifications=?,driver_routes=? WHERE truck_id=?",(np,nl,ni,nc,nr,tid))
